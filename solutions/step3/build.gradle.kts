...
tasks.register<CheckFileSize>("validateOutputSize") {
    group = "verification"
    description = "Ensures file is not too large"

    //dependsOn("createLargeOutputFile")
    maxFileSizeInBytes = 1024L * 1024L * 1L
    //fileToValidate = layout.buildDirectory.file("outputFile.pkg")
    fileToValidate = tasks.named<GenerateRandomFile>("createLargeOutputFile").get().outputFile
}
